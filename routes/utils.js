...
function simpleResponse(result, response) {
  ...
  if (success) {
    response.status(200);
  } else if (ecode == utils.errorEnum.SERVER) {
    response.status(500);
  } else if (
    ecode == utils.errorEnum.UNIQUE ||
    ecode == utils.errorEnum.FOREIGN
  ) {
    // Should only occur in queries that includes insert and update.
    response.status(409);
  } else if (ecode == utils.errorEnum.DNE) {
    // Should only occur in queries that includes select and delete.
    response.status(404);
  } else if (ecode == utils.errorEnum.INVALID) {
